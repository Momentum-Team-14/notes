...
### What do you expect the purpose of the `script` element is?
...
For when you need to represent characters, letters, or any kind of text.
...
```js
// ⚠️ variables declared with const cannot be reassigned
const width = 300
width = 400 // 👎 NOPE! You can't do this; you will get an error
const firstName = 'Dorian' // 👍 as long as you don't try to reassign a value to this variable

// variables declared with let can be reassigned
let points = 12
let paused = false
paused = true // 👍 OK to do this
points = 13 // 👍 totally OK
points = points + 2 // 👍 Also OK
```
...
5. Read the instructions in the `intro.js` file. You will need to replace or uncomment the necessary lines in each exercise as you work on it. When you make a change in the javascript file, save it and then reload the index.html page in the browser. You should see the `console.log()` output in the console.
...
2. a function called `sum` with two parameters that returns the sum of any 2 numbers passed in as arguments when the function is called
3. a function called `getLength` that takes one argument (a string) and returns its length
4. a function that takes a character as an argument (i.e., a string of length 1) and returns true if it is a vowel, false if it is not a vowel.
